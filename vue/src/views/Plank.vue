...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      if (this.form.id) {  // 更新
        request.put("/plank", this.form).then(res => {
          console.log(res)
          if (res.code === '0') {
            this.$message({
              type: "success",
        ...
        request.post("/plank", this.form).then(res => {
          console.log(res)
          if (res.code === '0') {
            this.$message({
              type: "success",
    ...
    handleEdit(row) {
      ...
      this.$nextTick(() => {
        ...
        if (!editor) {
          ...
          // 配置 server 接口地址
          editor.config.uploadImgServer = 'http://8.136.80.129:9090/files/editor/upload'
          editor.config.uploadFileName = "file"  // 设置上传参数名称
          editor.create()
        }
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/plank/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
